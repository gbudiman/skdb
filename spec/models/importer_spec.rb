...
RSpec.describe Importer, type: :model do
  ...
  context "validation" do
    it "should expect IndexError when same hero of different rank has identical name" do
      expect { Importer.test_stub_skills @data_with_hero_error }.to raise_error(IndexError)
    end

    it "should expect NameError when multiple skill name exists for one the same category" do
      expect { Importer.test_stub_skills @data_with_skill_error }.to raise_error(NameError)
    end

    it "should expect NameError when hero's skill of the same category has non-uniform name" do
      expect { Importer.test_stub_skills @data_with_non_uniform_skill_error}.to raise_error(NameError)
    end

    it "should expect IndexError when one attribute appears multiple time in a particular skill" do
      expect { Importer.test_stub_skills @data_with_duplicate_attribute}.to raise_error(IndexError)
    end
  end

  context "execution" do
    before :each do
      @imported_data = Importer.test_stub_skills @data
    end

    context "statistics" do
      it "should correctly reflect inserted heros" do
        expect { @imported_data.commit! }.to change{Hero.count}.by(@imported_data.result_skills[:heros].keys.count)
      end

      it "should correctly reflect inserted skills" do
        expect { @imported_data.commit! }.to change{Skill.count}.by(@imported_data.result_skills[:skills].keys.count)
      end

      it "should correctly reflect inserted attributes" do
        expect { @imported_data.commit! }.to change{Atb.count}.by(@imported_data.result_skills[:attributes].keys.count)
      end

      it "should correctly reflect inserted skill_attributes" do
        expect { @imported_data.commit! }.to change{SkillAtb.count}.by(@imported_data.result_skills[:skill_attribute_count])
      end
    end
    ...
    context "overwriting" do
      ...
      it "must be able to update hero" do
        ...
        Importer.test_stub_skills(new_data).commit!
        expect([old_row.name, Hero.find_by(static_name: 'rudy_4').name]).to eq [@imported_data.result_skills[:heros]['rudy_4'], 'Haxxed Guardian Rudy']
      end
      ...
      it "must be able to update skill" do
        ...
        Importer.test_stub_skills(new_data).commit!
        expect([old_row.name, Skill.find_by(static_name: 'rudy_4_0').name]).to eq [@imported_data.result_skills[:skills]['rudy_4_0'], 'Mega Rush']
      end
